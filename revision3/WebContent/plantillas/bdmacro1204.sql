...
--
-- Servidor: localhost
-- Tiempo de generación: 12-04-2015 a las 17:50:45
...
CREATE TABLE IF NOT EXISTS `ingresoproducto` (
  ...
  `vTipoCompra` varchar(5) DEFAULT NULL,
...
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COLLATE=utf8_spanish_ci AUTO_INCREMENT=266 ;
...
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COLLATE=utf8_spanish_ci AUTO_INCREMENT=75 ;
...
BEGIN
     ...
     /*  SELECT iFormaPagoId INTO xiFormaPagoId FROM VentaDevolucion  WHERE iVentaDevolucionId = xId ; */
         ...
         'ASIENTO N2','0',xiPeriodoId);
           ...
           /*****************/           
             ...
             IF xiFormaPagoId=1 THEN 
                   ...
                   /**Cambiamos el estado del documento**/
                   UPDATE VentaDevolucion set vEstadoDocumento='CANCELADO' where iVentaDevolucionId = xId;
                   
                   /**Registro de estado Cuenta proveedor**/             
                   INSERT INTO Estadocuentaproveedor(fMontoPago,
                   dFechaPago,dFechaInserta,
                   iUsuarioInsertaId,cEstadoCodigo,
                   iClienteId,iVentaDevolucionId)
               ...
               **/
               ...
               IF(xfMontoAdelantado !=0) THEN 
                 ...
                 * id=2, '1', '101', 'Caja'*/
                 ...
                 /**Cambiamos el estado del documento**/
                 /**Registro de estado Cuenta proveedor**/             
               INSERT INTO Estadocuentaproveedor(fMontoPago,dFechaPago,dFechaInserta, iUsuarioInsertaId,cEstadoCodigo,iClienteId,iVentaDevolucionId)
               VALUES(xfMontoAdelantado,NOW(),NOW(),xiUsuarioId,'AC',xiClienteId,xId);
          
                 /**Cambiamos el estado del documento**/
                 UPDATE VentaDevolucion set vEstadoDocumento='DEUDA' where iVentaDevolucionId = xId;
             ...
             WHILE x  <= xnumeroLetras DO 
               ...
               **/
               SELECT iPeriodoId INTO xProximoiPeriodoId FROM periodo WHERE  year(DFECHAINICIO)=year(fechaVencimiento) and MONTHNAME(DFECHAINICIO)=MONTHNAME(fechaVencimiento) ;
                ...
                INSERT INTO letraProveedor (iClienteId,nNumeroLetra,nNumeroUnico, dFechaGiro, dFechaVencimiento, nImporte, dFechaInserta,iUsuarioInsertaId,vEstadoLetra,cEstadoCodigo,iVentaDevolucionId) 
       ...
       /****************/	      	   
         ...
         vConceptoGeneral=CONCAT('COMPRA/ Mercaderia / NRO de ',tipoDocumento,': ',numeroFactura),
         iPeriodoId=xiPeriodoId
         WHERE iCuentasId=220 AND iVentaDevolucionId= xId AND iPeriodoId=xiPeriodoId;
         ...
         vConceptoGeneral=CONCAT('TRIBUTOS POR PAGAR / Gobierno central / IGV / NRO de ',tipoDocumento,': ',numeroFactura),
         iPeriodoId=xiPeriodoId
         WHERE iCuentasId=168 AND iVentaDevolucionId= xId AND iPeriodoId=xiPeriodoId;       	
         ...
         vConceptoGeneral='PROVEEDORES / Facturas, boletas y otros comprobantes por pagar',
         iPeriodoId=xiPeriodoId
         WHERE iCuentasId=199 AND iVentaDevolucionId= xId AND iPeriodoId=xiPeriodoId;
        ...
        UPDATE Librodiario SET
         ...
         vConceptoGeneral=CONCAT('MERCADERIA / Almacen / NRO de',tipoDocumento,': ',numeroFactura),
         iPeriodoId=xiPeriodoId
         WHERE iCuentasId=57 AND iVentaDevolucionId= xId AND iPeriodoId=xiPeriodoId;
         ...
         vConceptoGeneral=CONCAT('VARIACION DE EXISTENCIAS / Mercaderia / NRO de',tipoDocumento,': ',numeroFactura),
         iPeriodoId=xiPeriodoId
         WHERE iCuentasId=255 AND iVentaDevolucionId= xId AND iPeriodoId=xiPeriodoId;
           ...
           /****************/
             ...
             * id= '199', '4', '421', 'Facturas, boletas y otros comprobantes por pagar'**/  
             ...
             IF xiFormaPagoId=1 THEN 
              ...
              /************************************************************/
              /** cambiamos de esta a eliminado  QUE SE INSERTO EN ESTADO CUENTA PROVEEDOR**/              
              UPDATE  Estadocuentaproveedor SET cEstadoCodigo='EL' WHERE iVentaDevolucionId=xId;
              UPDATE  Librodiario SET cEstadoCodigo='EL' WHERE iVentaDevolucionId=xId AND iPeriodoId=xiPeriodoId
                        AND iCuentasId=2 AND vTipoConcepto='H';
            ...
            IF xiFormaPagoId=5 ||  xiFormaPagoId=6 THEN 
              ...
              /************************************************************/
              /** cambiamos de esta a eliminado  QUE SE INSERTO EN ESTADO CUENTA PROVEEDOR**/              
              UPDATE  letraProveedor SET cEstadoCodigo='EL' WHERE iVentaDevolucionId=xId;
              UPDATE  Librodiario SET cEstadoCodigo='EL' WHERE iVentaDevolucionId=xId AND iPeriodoId=xiPeriodoId
                        AND iCuentasId=201 AND vTipoConcepto='H';
