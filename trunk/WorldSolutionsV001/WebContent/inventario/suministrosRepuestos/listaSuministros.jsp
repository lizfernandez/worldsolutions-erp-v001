     ...
     <tr>
      <% 
	   for (String per: listapermiso) {
			if(per!=null){
	   if(per.equals("1231")){%>
        <td><button  class="button" onclick="popup('productos.do?metodo=mantenimientoProducto&iclasificacionId=3&mode=I',700,550)">
	   ...
	   for (String per: listapermiso) {
			if(per!=null){
	   if(per.equals("1233")){%>
        <td><button  class="button" onclick="popup('productos.do?metodo=mantenimientoProducto&mode=F&iclasificacionId=3',470,550)">
...
<logic:notEmpty name="productosForm" property="produc"> 
	...
	<tr>
		...
		<img title="Editar" src="${pageContext.request.contextPath}/media/imagenes/edit.png"
		                     onclick="popup('productos.do?metodo=mantenimientoProducto&iclasificacionId=3&mode=U&id=<bean:write name="x" property="iProductoId" />',700,550)" />  
		 <% break; }}}%>
		</td>
		<td align="center">
		<%
