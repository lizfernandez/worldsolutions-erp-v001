var site_url = '';
 ...
 var nav  = ['#inventario', '#cliente', '#proveedor', '#compra', '#venta', '#contabilidad', '#estadistica', '#admin'];
...
var bp=0;
var mf =6;
$("#navigation").find(".pri-nav").each(function(key,val){ 
	nav.push("#"+this.id);
	$("#"+this.id).css({"background-position":bp+"px -25px","margin-left":mf+"px"});
	bp=bp-98;
	mf=mf+99;
						...
						$(this)
							.children()
								.animate( {backgroundPosition:'(0px -45px)'},"fast",function(){
											...
											$(parent).css({backgroundImage: 'url(media/imagenes/nav_active.png)'});
									});
					});
			break;
		}
...
{
	for ( i = 0; i < nav.length; i++ ){
		/*$(nav[i]).css({backgroundPosition: i*(-98) + 'px -68px'});
		$(nav[i] + ' div').css({ backgroundPosition: '0px -60px'});*/
		
		$(nav[i]).css({backgroundPosition: i*(-98) + 'px 25px'});
		$(nav[i] + ' div').css({ backgroundPosition: '0px -45px'});
	}
}
...
function mMouseOver(e)
{
	...
	$(this)
		...
		.css({backgroundImage: 'url(media/imagenes/nav-over.png)',cursor: 'pointer'})
					...
					.children()
						...
						.animate( {backgroundPosition:'(0px -45px)'},"fast",
							...
							function(){
								...
								$(parent).css({backgroundImage: 'url(media/imagenes/nav.png)'});
							});
...
{			
	...
	$(this)
		...
		.css({backgroundImage: 'url(media/imagenes/nav.png)', cursor: ''});
}
